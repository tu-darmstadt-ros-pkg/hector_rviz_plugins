...
PointCloudFilterDisplay::PointCloudFilterDisplay()
    : point_cloud_common_( new rviz::PointCloudCommon( this ) ), cloud_q()
{
  ...
  filter_property_ =
      new rviz::BoolProperty( "Enable filtering", filtering, "Whether to enable filtering or not",
                              this, SLOT( enableFiltering() ), this );

  axis_property_ =
      new rviz::EnumProperty( "Filtered Axis", "Z", "Axis around which the points are filtered",
  ...
  min_value_property_ =
      new rviz::FloatProperty( "Min Value", -2.0, "Minimum value for points to be displayed.",
                               filter_property_, SLOT( updateParameters() ), this );

  max_value_property_ =
      new rviz::FloatProperty( "Max Value", 2.0, "Maximum value for points to be displayed.",
                               filter_property_, SLOT( updateParameters() ), this );

  frame_property_ = new rviz::TfFrameProperty(
      "Frame", nullptr, "The frame to which the points are displayed relatively.", filter_property_,
...
void PointCloudFilterDisplay::processMessage( const sensor_msgs::PointCloud2ConstPtr &msg )
{
  ...
  while ( !cloud_q.empty() && now_sec - cloud_q.front()->header.stamp.toSec() >
  ...
  if ( !context_->getFrameManager()->getTF2BufferPtr()->canTransform(
           selected_frame, msg->header.frame_id, msg->header.stamp, ros::Duration( 0.0 ) ) ) {
    ROS_DEBUG( "Selected frame %s is not available!", selected_frame.c_str() );
    return;
  }

  pcl_ros::transformPointCloud( selected_frame, *msg, *cloud,
                                *context_->getFrameManager()->getTF2BufferPtr() );
...
template<class T>
void PointCloudFilterDisplay::filterCloud( pcl::PassThrough<T> filter,

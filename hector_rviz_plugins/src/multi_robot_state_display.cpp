...
{
  topic_property_ = new rviz::RosTopicProperty( "topic", "/display_multi_robot_state",
                                                "hector_rviz_plugins_msgs/DisplayMultiRobotState",
                                                "The to listen for hector_rviz_plugins_msgs/DisplayMultiRobotState message.",
...
{
  if ( last_message_ == nullptr ) return;
  ...
  for ( auto &kvp : displays_ )
  {
    ...
    if ( header.stamp.isZero()) header.stamp = last_message_->header.stamp;
  ...
  if ( needs_state_update_ )
  {
    ...
    for ( const auto &robot : last_message_->robots )
    {
      if ( robot.id.empty())
      {
        ROS_ERROR_ONCE( "No id provided for robot state and the state was ignored! This message is printed once!" );
        continue;
      }
...
void MultiRobotStateDisplay::onNewMultiRobotState( const hector_rviz_plugins_msgs::DisplayMultiRobotStateConstPtr &msg )
{
  last_message_ = msg;
  ...
  for ( const auto &robot : msg->robots )
  {
    ...
    if ( it == displays_.end())
    {
      ...
      connect( it->second, &PrivateRobotStateDisplayHelper::changed,
               this, &MultiRobotStateDisplay::onSubdisplayEnableChanged );
      addChild( it->second );
...
{
  sub_.shutdown();
  sub_ = update_nh_.subscribe<hector_rviz_plugins_msgs::DisplayMultiRobotState>( topic_property_->getTopicStd(), 10,
                                                                                 &MultiRobotStateDisplay::onNewMultiRobotState,
                                                                                 this );
}

void MultiRobotStateDisplay::onSubdisplayEnableChanged()
{
  needs_state_update_ = true;
}
